...
*///
Vector3D calibrate(sensors_event_t accelEvent,sensors_event_t magEvent) 
{
  ...
  double hardiron_x = (MagMaxX + MagMinX) / 2;
  double hardiron_y = (MagMaxY + MagMinY) / 2;
  double hardiron_z = (MagMaxZ + MagMinZ) / 2;

  return Vector3D(hardiron_x,hardiron_y,hardiron_z)
